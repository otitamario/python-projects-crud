...
def get_projects(
    ...
    projects = (
        ...
        .filter(models.Project.title.contains(search))
        .limit(limit)
        .offset(skip)
        .all()
    )
...
@router.post(
    "/", status_code=status.HTTP_201_CREATED, response_model=schemas.ProjectResponse
)
def create_project(
    project: schemas.CreateProjectSchema,
    db: Session = Depends(get_db),
    owner_username: str = Depends(require_user),
):
...
def update_project(
    id: str,
    project: schemas.UpdateProjectSchema,
    db: Session = Depends(get_db),
    username: str = Depends(require_user),
):
...
def get_project(
    id: str, db: Session = Depends(get_db), username: str = Depends(require_user)
):
